...
#     """
#     Генерация рекомендаций на основе запроса с использованием kmeans_model
#     и эмбеддингов из RuBERT
#     """
#     try:
#         # Получаем эмбеддинг для запроса
#         query_embedding = get_embeddings(query).numpy()

#         # Приводим размерность эмбеддинга к целевой размерности кластерной модели
#         target_dim = kmeans_model.cluster_centers_.shape[1]
#         query_embedding_resized = resize_embedding(query_embedding, target_dim)

#         # Определяем кластер для запроса
#         query_cluster = kmeans_model.predict(query_embedding_resized)[0]

#         # Фильтруем данные и выбираем рекомендации
#         recommended_items = data[data['cluster'] == query_cluster].head(5)
